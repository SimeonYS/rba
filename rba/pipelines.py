...
class RbaPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('rba.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `rba`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM rba WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO rba (date, title, link, content)"
